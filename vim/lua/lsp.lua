return {
  on_attach = function(client, bufnr)
    ...
    if client.server_capabilities.typeDefinitionProvider then vim.keymap.set('n', 'gt', vim.lsp.buf.type_definition,
      bufopts) end
    if client.server_capabilities.implementationProvider then vim.keymap.set('n', 'gi', vim.lsp.buf.implementation,
      bufopts) end

    if client.server_capabilities.hoverProvider then vim.keymap.set('n', 'K', vim.lsp.buf.hover, bufopts) end
    if client.server_capabilities.signatureHelpProvider then vim.keymap.set({ 'i', 'n' }, '<C-k>',
      vim.lsp.buf.signature_help, bufopts) end

    if client.server_capabilities.documentFormattingProvider then vim.keymap.set('n', 'gqq',
      function() vim.lsp.buf.format { async = true } end, bufopts) end
    if client.server_capabilities.renameProvider then vim.keymap.set('n', '<localleader>rn', vim.lsp.buf.rename, bufopts) end
    if client.server_capabilities.codeActionProvider then vim.keymap.set('n', '<localleader>ca', vim.lsp.buf.code_action
      , bufopts) end
  end
}
